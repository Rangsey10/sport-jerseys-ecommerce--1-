...
export function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    try {
      console.log('🔄 Admin dashboard - Starting order load...')
      setLoading(true)
      
      // Use order service
      const allOrders = await orderService.getAllOrders()
      console.log('📊 Order service result:', {
        ordersLength: allOrders?.length || 0,
        sampleOrder: allOrders?.[0]
      })
      
      setOrders(allOrders || [])
    } catch (error) {
      console.error('❌ Error loading orders in admin dashboard:', error)
      setOrders([])
    } finally {
      setLoading(false)
    }
            ...
            <CardHeader>
              ...
              {orders.length > 0 && (
                <p className="text-sm text-gray-600">
                  👥 <strong>{orders.length}</strong> customers have placed orders. 
                  💰 Total revenue: <strong>${orders.reduce((sum, order) => sum + order.total_amount, 0).toFixed(2)}</strong>
                </p>
              )}
            </CardHeader>
            <CardContent>
              ...
              {loading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <div className="space-y-2 text-sm text-gray-500">
                    <p>💡 To test orders:</p>
                    <p>1. Visit your store as a customer</p>
                    <p>2. Add items to cart and checkout</p>
                    <p>3. Orders will appear here in real-time</p>
                  </div>
                  <div className="mt-6">
